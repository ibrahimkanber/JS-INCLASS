...
function loadArticles(data){
    ...
    data.forEach((d)=>{
        ...
        articleEl.innerHTML=`<p class="title">${d.title}</p>
                            <p>${d.desc}</p>
...
const input=document.querySelector("#count_input");
document.getElementById("search_button").addEventListener("click",getInput)


function getInput(){
    filterArticles(input.value)
    // input.value==""? loadArticles(data):
}

function filterArticles(count){
    const filteredData=data.filter((item)=>item.readCount>count);
    console.log(filteredData);
    loadArticles(filteredData)
}
